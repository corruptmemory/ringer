...
> **STATUS: EXECUTED 2026-07-10** (branch `go-hud`, 9 tasks + fixes, opus whole-branch review = READY TO MERGE after one Critical fix). **Shipped code is authoritative over the snippets below** where they drift. Corrections found during execution:
> - **Layout (Task 4)** — the plan's `layout.templ` set `hx-swap="morph"` on the polling panels; that (outerHTML morph) makes idiomorph *hard-replace* the polling `<section>` with the fragment's `<div>`, discarding `hx-get`/`hx-trigger` — so the HUD renders once and never live-updates (the whole headline feature dead). **All per-task reviews missed it** (httptest can't catch it; the test even pinned the broken value). The opus final review browser-verified it and the shipped code uses **`hx-swap="morph:innerHTML"`** (morphs the section's children, preserving the polling container + scroll/expanded state). Commit fe8799b.
> - **Runs templ (Task 6)** — the plan's `runs.templ` used invented CSS classes (`work-list`, `work-item`, standalone `muted`) that don't exist in the lifted `ringside.css`, so task rows rendered unstyled. Shipped `runs.templ` is realigned to the real ringside classes (`worker`/`workers`/`glyph`/`state`/`run`/`corner`/`run-stats`/`activity` — all verified present as selectors).
> - **Task 9 test** — the plan's `ensurehud_test.go` fork-bombs under `go test` (`os.Executable()` is the test binary → `spawnDetachedHUD` re-execs it as `hud` → the whole package, including that test, re-runs recursively). Shipped code adds a `TestMain` guard (`os.Args[1]=="hud"` → `os.Exit(0)`) that breaks the recursion; production `ensurehud.go` is unchanged (re-execing the *real* ringer is intended). Commit 6ed5fc0.
> - **Fragment routes** — the plan named the handlers `handleRuns`/`handleLibrary`/etc.; a transient `stubs.go` (created in Task 4, trimmed per-task, `git rm`'d in Task 8) let each task compile against the router before its handler existed.
>
> **Accepted whole-branch-review Minors (shipped, non-blocking):** tmp-file leak if `os.Rename` fails (in both `artifact/library.go` and the precedented `state/state.go`); `FormatDuration` rounds before the `<60s` branch (59.6s→"1m 00s"); `handleLibrary` re-reads library.json every poll (redundant); dead/redundant guards in `taskLogPath`/open-folder; `spawnDetachedHUD` never `Wait()`s the detached child (intended daemonization); no e2e test that `--dry-run` skips ensure-running; the topbar `#clock`/`#top-dot` are static (the lift dropped the JS updater — cosmetic); cross-process `active-runs.json` lost-update is inert until the Plan-4b renderer writes live library entries.
>
> **Out of scope → Plan 4b (artifact rendering) + Plan 5 (analytics, /hud/models data, cutover).** See the "Out of scope" section at the end.
